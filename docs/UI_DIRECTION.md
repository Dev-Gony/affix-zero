...
새 04는 왼쪽 노드·오른쪽 선택 상세와 명시적 투자, 새 06은 왼쪽 대상·중앙 강화 미리보기/실행·오른쪽 속성의 정보 구조다. 현재는 실제 3노드 분기와 단일 장착 무기 확정 강화를 연결했다. 05 펫은 후속 참조다. 시안의 예시 재화·확률·원격 이미지를 게임 데이터나 사용 가능한 에셋으로 간주하지 않는다. 최신 검수는 `validation/management`, 시각 승인은 미완료다.
