...
| 문제 발생 지점 | 원인 분석 | 해결 방법 및 적용된 코드 개념 | 배운 점 |
|---|---|---|---|
| 기능은 연결됐지만 시안과 정보 위계가 다름 | 장착카드·영웅·능력치·비교의 상대 크기와 배치 반영이 부족 | 기존모델/명령 유지, 네이티브영역·헤더·아이콘·수치·행동버튼 재배치, 두해상도 실제캡처검수 | 기능PASS와 시각검수를 분리한다 |
| 새 UI시험fixture가 실제저장에 들어갈 위험 | 통상게임과 자동검사의 저장경로를 공유할 수 있음 | explicit UIflag는 항상 메모리프로필, 다른probe와동시실행거절, FilePath빈값/SaveCount0검사 | 시각검사용아이템 공급은 실제플레이/드랍성적과 구분한다 |
