...
func _run() -> void:
	...
	basic_projectile.setup(Vector2.ZERO, dummy_enemy, {"damage": 10, "critical": false}, Color("b56dff"), "arcane")
	_check(basic_projectile.target == dummy_enemy, "Ranged basic attacks bind to a visible enemy projectile target")
	_check(basic_projectile.projectile_style == "arcane", "Mage basic attacks keep an arcane projectile identity")
	var sage_projectile := PlayerBasicProjectile.new()
	add_child(sage_projectile)
	sage_projectile.setup(Vector2.ZERO, dummy_enemy, {"damage": 10, "critical": false}, Color("c9a7ff"), "lightning")
	_check(sage_projectile.projectile_style == "lightning" and sage_projectile.speed > basic_projectile.speed, "Sage basic attacks use faster lightning visuals")
	sage_projectile.queue_free()
