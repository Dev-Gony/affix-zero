...
func _run() -> void:
	...
	_check(new_midgame_drop <= 2.0 and new_midgame_drop >= 0.8, "Field item drop chance stays inside the intended 0.8-2.0 percent band")
	...
	_check(is_equal_approx(GameManager.equipment_enhancement_success_rate(30), 0.01), "+30 enhancement is an extreme 0.01 percent wall")
	_check(GameManager.equipment_enhancement_destroy_chance(20) >= 20.0, "+20 failure can destroy equipment")
	_check(GameManager.equipment_enhancement_destroy_chance(30) >= 60.0, "+30 failure is catastrophically dangerous")
	var cost_item := {"item_level": 60, "enhancement_level": 0}
	var plus_one_cost: int = GameManager.equipment_enhancement_cost(cost_item)
	...
	_check(GameManager.class_skill_max_level() == 40, "Each rebirth expands the class skill cap by ten levels")
	...
	if not definitions.is_empty():
		...
		var level_zero_cost: int = GameManager.skill_upgrade_cost_for_level(base_cost, cost_step, 0)
		var level_twenty_cost: int = GameManager.skill_upgrade_cost_for_level(base_cost, cost_step, 20)
		_check(level_twenty_cost > level_zero_cost * 100, "Skill costs scale aggressively enough to remain a late-game gold sink")
		...
		_check(GameManager.class_skill_level(skill_id) <= GameManager.class_skill_max_level(), "Bulk upgrading never exceeds the rebirth-scaled class skill cap")
